...
export function redrawCanvas() {
  ...
  for (let i = 0; i < global.drawing.length; i++) {
    for (let j = 0; j < global.drawing[i].data.length; j++) {
      ...
      //console.log(global.drawing[i].data[j].strokeStyle);
      drawline(
        toscreenX(line.x0),
        toscreenY(line.y0),
        toscreenX(line.x1),
...
function iS_on_Shape(x, y, shape) {
  ...
  let shape_right = toscreenX(shape.x) + currWidth(shape.width);
  let shape_top = toscreenY(shape.y);
  let shape_bottom = toscreenY(shape.y) + currHeight(shape.height);
